...
<body>
  <nav class="navbar">
    <div class="nav-container">
      ...
      <div class="nav-menu" id="navMenu">
        ...
        <a href="admin_users.php" class="nav-link admin-only">Users</a>
        <a href="create_tourna.php" class="nav-link admin-only">Tournaments</a>
  ...
  <main class="main-content">
    <div class="container">
      <div class="page-header">
        <h2>Dashboard</h2>
        <p>
          Welcome back, <span id="userName"><?php echo htmlspecialchars($userName); ?></span>!
          (<span id="userRole"><?php echo htmlspecialchars($userRole); ?></span>)
        </p>
      </div>

      <!-- MEMBER STATISTICS -->
      <div id="memberStats" class="stats-grid member-only"
        style="display: <?php echo $userRole === 'Member' ? 'grid' : 'none'; ?>">
      ...
      <!-- ADMIN STATISTICS -->
      <div id="adminStats" class="stats-grid admin-only"
        style="display: <?php echo $userRole === 'Admin' ? 'grid' : 'none'; ?>">
        <div class="stat-card stat-green">
          <div class="stat-content">
            <div class="stat-icon">📊</div>
      ...
      <div id="adminExtraStats" class="stats-grid admin-only"
        style="display: <?php echo $userRole === 'Admin' ? 'grid' : 'none'; ?>; margin-top: 24px">
        <div class="card">
        ...
        <div class="card">
          ...
          <p class="stat-number" id="adminRevenueMonth">₱0</p>
      ...
      <div class="dashboard-grid">
        ...
        <div class="card">
          <h3 class="card-title">
            <span class="icon">📅</span>
            <span id="bookingsTitle"><?php echo $userRole === 'Admin' ? 'All Bookings' : 'My Recent Bookings'; ?></span>
          </h3>
  ...
  <script>
    ...
    // Add body class based on role for CSS styling
    document.body.classList.add(currentUser.role.toLowerCase());
    ...
    function displayCourts(courts) {
      ...
      grid.innerHTML = courts
        .map(
          (court) => `
                <div class="court-card ${court.availability_status === "Available"
              ? "available"
              : "occupied"
            }">
          ...
          (booking) => `
                <div class="booking-item">
                    <div class="booking-info">
                        ...
                        <span class="badge badge-${booking.status.toLowerCase()}">${booking.status
            }</span>
                    </div>
                    <p class="booking-time">⏰ ${formatTime(
              booking.start_time
            )} - ${formatTime(booking.end_time)}</p>
                    <p class="booking-time" style="font-size: 12px; color: #9ca3af;">📅 ${formatDate(
